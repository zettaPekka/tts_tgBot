...
async def voice_handler(callback: CallbackQuery, user_service: UserService):
    ...
    voice = user.voice.title() if user.voice else 'не выбран\n\nНеобходимо указать голос для озвучки'
    
    await callback.message.answer(f'<b>⚡️ Выберите опцию для продолжения\n\nВыбранный голос: {voice}</b>',
...
async def choose_voice(callback: CallbackQuery):
    await callback.answer()
    try:
        await callback.message.edit_text('<b>🧍 Выберите пол для голоса</b>',
                                        reply_markup=user_kbs.gender_kb)
    except:
        await callback.message.answer('<b>🧍 Выберите пол для голоса</b>',
                                        reply_markup=user_kbs.gender_kb)
...
async def choose_voice(callback: CallbackQuery):
    ...
    await callback.message.edit_text(message_text,
                                    reply_markup=await user_kbs.voices(gender))
...
async def set_voice(callback: CallbackQuery, user_service: UserService):
    ...
    await callback.message.edit_text(f'<b>Голос {voice_id.title()} успешно выбран!</b>',
                                        reply_markup=user_kbs.start_voice)


@router.callback_query(F.data == 'input_text')
async def input_text_handler(callback: CallbackQuery, state: FSMContext, user_service: UserService):
    await callback.answer()
    
    user = await user_service.get(callback.from_user.id)
    
    await callback.message.answer(f'<b>Введите текст для озвучки, который не превышает 2000 символов\n\n<blockquote>1 ⭐️ - 150 символов\nВаш баланс {user.balance}⭐️\nХватит на {user.balance * 150} символов</blockquote></b>',
...
async def generate_audio(message: Message, state: FSMContext, user_service: UserService):
    if message.content_type != ContentType.TEXT or len(message.text) >= 2000:
        await message.answer('<b>Не верные входные данные для озвучки, возможно текст длинее 2000 символов, попробуйте еще раз</b>')
        return
    
    user = await user_service.get(message.from_user.id)
    
    if len(message.text) > user.balance * 150:
        await message.answer('<b>У вас недостаточно средств для озвучки текста такой длины, пополните баланс</b>',
    ...
    if not user.voice:
        await message.answer('<b>Упс, кажется вы еще не выбрали голос</b>',
    ...
    try:
        audio_path = await speechify_text_to_speach.text_to_speach(message.text, user.voice, message.from_user.id)
    except:
    ...
    await waiting_message.answer_audio(audio, caption='<b>⚡️ Все готово! Аудио с вашим текстом сгенерировано, хотите озвучить новый текст?</b>',
                                        reply_markup=user_kbs.voice_kb)
...
async def back_handler(callback: CallbackQuery, state: FSMContext, user_service: UserService):
    await callback.answer()
    
    user = await user_service.get(callback.from_user.id)
    photo = FSInputFile('images/start.jpg')
    
    await callback.message.answer_photo(photo, caption=f'<b>👋 Привет, здесь ты можешь озвучить свой текст разными голосами людей\n\nВаш баланс: {user.balance}⭐️\nХватит на: {user.balance * 150} символов\n\n<blockquote>Качественная озвучка\nБыстрая обработка\nНизкие цены (1 ⭐️ - 150 символов)\nОкругление символов по законам математики\nДо 2000 символов за 1 запрос</blockquote>\n\nИспользуй кнопки ниже👇</b>', 
                            reply_markup=user_kbs.start_kb)
    await state.clear()
...
async def back_handler(callback: CallbackQuery, user_service: UserService):
    ...
    voice = user.voice.title() if user.voice else 'не выбран\n\nНеобходимо указать голос для озвучки'
    await callback.message.edit_text(f'<b>⚡️ Выберите опцию для продолжения\n\nВыбранный голос: {voice}</b>',
                                    reply_markup=user_kbs.voice_kb)
