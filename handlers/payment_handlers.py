...
async def deposit(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "<b>Пополнение баланса производится через Telegram Stars, введите ниже сумму для пополнения баланса.\n<blockquote>1 ⭐️ - 150 символов</blockquote></b>",
...
async def send_check(message: Message, state: FSMContext):
    if (
        ...
        or int(message.text) > 999999
    ):
        await message.answer(
            "<b>Неверная сумма для пополнения, попробуйте еще раз</b>",
            reply_markup=user_kbs.back_kb,
        )
        return
